...
$(document).ready(function () {
  ...
  var formatDate = function (d) {
    ...
    switch (
      d[2].substring(1) //We can use the string of the day (dd format) to access the last number, which determines what we append...
    ) {
      case "1":
  ...
  $("#button").click(function () {
    ...
    if (form_validated) {
     ...
     /* $.post( "Expense_adder", { name: $name.val(), time: formatDate($date.val()),amount: $amount.val(), unit_id : $u_id.val(),property_id: $p_id.val()  })
      .done(function() {
       console.log("added theree");
      });*/
      
      $.ajax({
    	  type: "POST",
    	  url: "Expense_adder.jsp",
    	  data: { name: $name.val(), time: formatDate($date.val()),amount: $amount.val(), unit_id : $u_id.val(),property_id: $p_id.val()  }
    	});
      /*var ob=new XMLHttpRequest();
      ob.open("POST","/temp/Expense_adder",true);
      ob.send();*/
